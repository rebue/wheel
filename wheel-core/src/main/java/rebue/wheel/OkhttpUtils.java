...
//			urlBuilder.addQueryParameter(item.getKey(), value);
	 ...
	 * @return 响应的字符串
	 */
	public static String postByFormParams(final String url, final Map<String, Object> requestParams)
			throws IOException {
		_log.debug("发送请求：{}", url);
		final FormBody.Builder formBodyBuilder = new FormBody.Builder();
		for (final Map.Entry<String, Object> item : requestParams.entrySet()) {
	 ...
	 * @return 响应的字符串
	 */
	public static Map<String, Object> postByXmlParams(final String url, final Map<String, Object> requestParams)
	...
	public static String putByJsonParams(final String url, final String jsonParams) throws IOException {
		_log.debug("发送请求：{}: {}", url, jsonParams);
		final Request request = new Request.Builder().url(url)
				.put(RequestBody.create(MediaType.parse("application/json; charset=utf-8"), jsonParams)).build();
		final Response response = _client.newCall(request).execute();
		if (response.isSuccessful()) {
			final String msg = response.body().string();
			_log.debug("接收到response的信息：{}", msg);
	 ...
	 * @return 响应的字符串
	 */
	public static String deleteByFormParams(final String url, final Map<String, Object> requestParams)
			throws IOException {
		_log.debug("发送请求：{}", url);
		final FormBody.Builder formBodyBuilder = new FormBody.Builder();
		for (final Map.Entry<String, Object> item : requestParams.entrySet()) {
