...
public class CaUtils {
    ...
    public static Certificate generateCertificate(String signAlgorithm,
        ...
        certificateBuilder.addExtension(Extension.basicConstraints, true, new BasicConstraints(true));    // ca cert
        // 如果false为实体证书
        ...
        certificateBuilder.addExtension(Extension.authorityKeyIdentifier, false,
                extUtils.createAuthorityKeyIdentifier(subjectPublicKeyInfo)); // 授权密钥标识
        certificateBuilder.addExtension(Extension.subjectKeyIdentifier, false,
                extUtils.createSubjectKeyIdentifier(subjectPublicKeyInfo));     // 使用者密钥标识
     ...
     */
    @SneakyThrows
    public static PKCS10CertificationRequest generateCsr(String certAlias, File storeFile, char[] storePassword,
        ...
        SubjectPublicKeyInfo     subjectPublicKeyInfo    = SubjectPublicKeyInfo.getInstance(publicKey.getEncoded());
        CertificationRequestInfo csrInfo                 = new CertificationRequestInfo(subject, subjectPublicKeyInfo,
                new DERSet());
        ...
        byte[]                   sign                    = SignUtils.sign(certificate.getSigAlgName(), privateKey, data,
                initParams);
        return new PKCS10CertificationRequest(
                new CertificationRequest(csrInfo, signAlgorithmIdentifier, new DERBitString(sign)));
    }
    ...
    // * @param subject 使用者
    // * @param signAlgorithm
    // 签名算法(所谓签名其实就是先进行摘要，再对摘要进行加密，所以签名算法的名称一般为摘要算法名称+"with"+加密算法名称)
    // * @param keyPair 密钥对
    // * @param initParams 签名算法的初始化参数
    // * @return PKCS10格式的证书签名请求
    // */
    // @SneakyThrows
    // public static PKCS10CertificationRequest generateCsr(X500Name subject, String
    // signAlgorithm, KeyPair keyPair, AlgorithmParameterSpec initParams) {
    // PrivateKey privateKey = keyPair.getPrivate();
    // PublicKey publicKey = keyPair.getPublic();
    //
    // SubjectPublicKeyInfo subjectPublicKeyInfo =
    // SubjectPublicKeyInfo.getInstance(publicKey.getEncoded());
    // CertificationRequestInfo info = new CertificationRequestInfo(subject,
    // subjectPublicKeyInfo, new DERSet());
    // byte[] data = info.getEncoded(ASN1Encoding.DER);
    // AlgorithmIdentifier signAlgorithmIdentifier =
    // getSignAlgorithmIdentifier(subjectPublicKeyInfo.getAlgorithm());
    ...
    // signAlgorithmIdentifier, new DERBitString(sign)));
    // }
}
