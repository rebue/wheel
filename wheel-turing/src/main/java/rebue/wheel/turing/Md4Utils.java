...
public class Md4Utils {
	...
	public static void copy64(int M[], byte in[], int offset) {
		int i;
		for (i = 0; i < 16; i++) {
			M[i] = ((in[offset + i * 4 + 3] << 24) & 0xFF000000) | ((in[offset + i * 4 + 2] << 16) & 0xFF0000) | ((in[offset + i * 4 + 1] << 8) & 0xFF00) | (((int) in[offset + i * 4 + 0]) & 0xFF);
		}
	}
	...
	public static byte[] getHashByMd4(byte in[]) {
	...
	private static String getTestResult(String s) throws UnsupportedEncodingException {
		return BytesUtils.toHexString(getHashByMd4(s.getBytes("UnicodeLittleUnmarked")));
	}
